  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const c = new ethers.Contract(
        ...
        (Array.isArray(Pay2AlphaAbi)
          ? Pay2AlphaAbi
          : (Pay2AlphaAbi as any).abi) as any,
        signer
      );
      const price = ethers.parseUnits(String(formData.pricePerMessage || 0), 6);
      const me = await signer.getAddress();
      const current = await c.experts(me);
      let tx;
      if (current && current.name && current.name.length > 0) {
        tx = await c.setPrice(price);
      } else {
        tx = await c.registerExpert(formData.name, price);
      }
      await tx.wait();
      alert(
        current && current.name && current.name.length > 0
          ? "Price updated"
          : "Expert registered"
      );
      onClose();
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFormData((prev) => ({
                  ...prev,
                  pricePerMessage: parseInt(e.target.value),
                }))
              }
              className="glass-input w-full px-4 py-3 rounded-lg text-white placeholder-blue-200/50"
              placeholder="50"

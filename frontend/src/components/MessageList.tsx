...
const MessageList: React.FC<MessageListProps> = ({
  ...
  const totalValue = encryptedMessages.reduce(
    (sum, msg) => sum + (msg.cost || expertPrice),
    0
  );
    ...
    <div className="space-y-6">
        ...
        {encryptedMessages.length === 0 ? (
          <div className="text-center py-6">
            ...
            <p className="text-blue-200/60 text-sm">
              No encrypted messages available
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            ...
            {encryptedMessages.map((message) => (
              <div
                key={message.id}
                className="glass p-4 rounded-lg border border-blue-400/20"
              >
                ...
                <div className="text-blue-200/70 text-xs mb-3 italic">
                  "This message contains valuable insights about DeFi strategies
                  and risk management..."
                </div>

                <button
                  onClick={() =>
                    onRevealMessage(message.id, message.cost || expertPrice)
                  }
      ...
      <div className="glass-card p-4 rounded-xl web3-glow">
        ...
        <button
          ...
          className="w-full glass-button text-white px-4 py-2 rounded-lg font-medium web3-glow"
        >
          Buy Credits
        </button>
      </div>
      {showBuyCredits && (
        <BuyCreditsForm
          expert={{
            id: expertId,
            name: `${expertId.slice(0, 6)}...${expertId.slice(-4)}`,
